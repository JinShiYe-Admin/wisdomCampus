	...
	var logoOption = {
		...
		uptoken_func: function(file) { // 在需要获取 uptoken 时，该方法会被调用
			...
			if(uptokenData && uptokenData.code) { //成功
				...
				logoUploader.stop();
				layer.confirm(uptokenData.message, {
					icon: 2,
					title: '提示'
				});
			}
		...
		init: {
			...
			'FileUploaded': function(up, file, info) {
				...
				if(info.status == 200) {
					...
					var data = {
						...
						colv: uptokenData.data.Data[0].Domain + uptokenData.data.Data[0].Key
					}
					changeWebsiteConfig(data);
				} else {
					layer.confirm(JSON.stringify(info), {
						icon: 2,
						title: '提示'
					});
				}
			...
			'Error': function(up, err, errTip) {
				//上传出错时,处理相关的事情
				layer.closeAll('loading');
				layer.confirm(pluploadutil.errMes(err.code, errTip), {
					icon: 2,
					title: '提示'
				});
			},
	...
	unitWebsitePro(tempData, function(data) {
		...
		if(data.RspCode == 0) {
			if(data.RspData.length > 0) {
				...
				layer.confirm('没有获取到配置信息', {
					icon: 2,
					title: '提示'
				});
			}
		} else {
			layer.confirm("获取配置信息失败(" + data.RspTxt + ")", {
				icon: 2,
				title: '提示'
			});
		}
...
function changeWebsiteConfig(change) {
	...
	unitWebsitePro(commit, function(data) {
		if(data.RspCode == 0) { //成功
			...
			layer.closeAll('loading');
			layer.confirm("修改失败(" + data.RspTxt + ")", {
				icon: 2,
				title: '提示'
			});
		}
